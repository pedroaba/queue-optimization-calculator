...
interface ResultDisplayMMSPrioridadeProps {
  results: MMSPrioridadeResult
}

export function ResultDisplayMMSPrioridade({
  results,
}: ResultDisplayMMSPrioridadeProps) {
    ...
    <Card className="bg-white/5 border-white/10 backdrop-blur-sm">
      <CardHeader>
        <CardTitle className="text-white flex items-center gap-2">
          ...
          Resultados M/M/s com Prioridade
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-gray-300">
                ...
                <div>
                  ...
                  {formatNumber(results.W_barra_agregado[i])}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="p-4 rounded-lg bg-gray-800/50 border border-gray-700">
          ...
          <div className="text-xs text-gray-300 space-y-1">
            <div>ρ = λ / (s × μ)</div>
            <div>Pq = fórmula de Erlang C</div>
            <div>W̄ = W padrão da M/M/s = Wq + 1/μ</div>
            <div>Wₖ = (1 / pₖ) × (W̄ₖ - soma ponderada Wⱼ anteriores)</div>
            <div>Wqₖ = Wₖ - (1 / μ)</div>
            <div>Lₖ = λ acumuladoₖ × Wₖ</div>
            <div>Lqₖ = Lₖ - (λ acumuladoₖ / μ)</div>
          </div>
        </div>
      </CardContent>
    </Card>
