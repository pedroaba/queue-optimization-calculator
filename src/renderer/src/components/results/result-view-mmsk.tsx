...
import { Users, Clock, Activity, Target, Timer, Ban } from 'lucide-react'
...
type MMSKResult = {
  ...
  lambda_bar: number
  P_n_K: number
  P_n_greater_K: number
  P_W_greater_3: number
  P_Wq_greater_2: number
}

type ResultViewMMSKProps = {
  data: MMSKResult
  K: number
}

export function ResultViewMMSK({ data, K }: ResultViewMMSKProps) {
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-xl border border-zinc-600/30 hover:border-chart-2/50 transition-all duration-300">
                ...
                <div className="text-2xl font-bold text-zinc-50 mt-2">
                  {(data.P_n_K * 100).toFixed(2)}%
                </div>
              </div>

              {/* Overflow: Probabilidade de n > K (deve ser 0) */}
              <div className="p-4 rounded-xl border border-zinc-600/30 hover:border-chart-2/50 transition-all duration-300">
                ...
                <div className="text-2xl font-bold text-zinc-50 mt-2">
                  {(data.P_n_greater_K * 100).toFixed(2)}%
                </div>
              </div>
