...
export function MG1FormCalculator({ onCalculate }: FormProps) {
  ...
  const handleCalculate = () => {
    if (parameters.lambda <= 0 || parameters.mu <= 0 || parameters.sigma < 0) {
      toast.error('Erro de validação', {
        description:
          'As taxas de chegada (λ), atendimento (μ) devem ser maiores que zero, e o desvio padrão σ não pode ser negativo.',
      })
      return
    }
    try {
      const results = mg1(parameters)
      onCalculate(results)
      toast.success('Cálculo realizado com sucesso', {
        description: 'Os resultados foram calculados com sucesso!',
      })
    } catch (error) {
      console.log('Erro ao calcular M/G/1:', error)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        {/* σ */}
        <div className="bg-slate-700/30 p-4 rounded-lg border border-slate-600/30">
          <Label htmlFor="sigma" className="text-gray-200 font-medium">
            Desvio padrão do tempo de serviço (σ) *
          </Label>
          <Input
            id="sigma"
            step="0.1"
